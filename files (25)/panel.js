...
function vistaVentas(estado) {
  ...
  html += "<div class='fila' style='margin-bottom:12px'>" +
    "<button class='btn btn-principal' data-accion='venta-puerta'>Cargar venta en puerta</button></div>";
...
function tarjetaPedido(p, estado) {
  ...
  if (p.estado === "nuevo") {
    html += btn("confirmar-venta", p.id, "Confirmar venta", "btn-principal");
    html += btn("editar-pedido", p.id, "Editar pedido", "btn-suave");
    html += btn("rechazar-pedido", p.id, "Rechazar", "btn-suave");
  }
  /* Los retiros no pasan por Envíos, así que se cierran desde acá. */
  if (p.estado === "confirmado" && p.tipoEntrega === "retiro") {
    if (!p.cobrado) html += btn("retirado-cobrado", p.id, "Retirado y cobrado", "btn-principal");
    html += btn("retirado", p.id, "Marcar retirado", "btn-principal");
  }
  if (p.estado === "confirmado" || p.estado === "finalizado") {
    if (!p.cobrado) html += btn("cobrar", p.id, "Marcar como cobrado", "btn-suave");
    html += btn("cancelar-venta", p.id, "Cancelar venta", "btn-suave");
  }
  if (p.estado !== "rechazado" && p.estado !== "cancelado") {
    html += btn("imprimir", p.id, "Imprimir comanda", "btn-suave");
    html += btn("cambiar-fecha", p.id, "Cambiar fecha", "btn-suave");
  }
  if (p.telefono) {
    html += "<a class='btn btn-suave btn-chico' target='_blank' rel='noopener' href='" +
